...
const generateCards = (gridSize: '4x4' | '6x6', theme: 'numbers' | 'icons') => {
  ...
  if (theme === 'icons') {
    values = icons.slice(0, totalCards / 2).map((icon) => icon.url); // Get the correct number of icons
  }
...
const Board: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setHighlightCards([]);
    setIsUsersFinished(false); // Ensure modal is not open at game start
    setIsTie(false); // Reset tie state at game start
    setTime(0); // Reset the timer at game start
  ...
  const handleCardClick = (index: number) => {
    ...
    if (revealedCards.length === 1) {
      ...
      if (cards[firstIndex] === cards[index]) {
        ...
        dispatch({type: 'INCREMENT_MOVES'}); // Increment moves if cards match
        ...
        setTimeout(() => {
          ...
          dispatch({type: 'INCREMENT_MOVES'}); // Increment moves if cards don't match
        }, 700);
      }
    }
  };
